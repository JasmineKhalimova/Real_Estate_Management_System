...
function renderForm($id, $Name, $Phone, $Address_Name, $Street, $City, $Province,$Country, $Postal_Code,$RealtorID,$RealtorIDResult,$error)
  ...
  <ul class="nav" id="nav" aria-hidden="true">
    ...
    <li>
      ...
      <ol>
        ...
        <li><a href="../client_payments/view-client-payment.php">Assign Payment</a></li>
...
while($row = mysqli_fetch_array($RealtorIDResult))
...
$RealtorIDResult = mysqli_query($conn, $query); 
...
renderForm($id, $Name, $Phone, $Address_Name, $Street, $City, $Province,$Country, $Postal_Code,$RealtorID,$RealtorIDResult, $error);
...
mysqli_query($conn, "UPDATE Client c INNER JOIN Address a ON (c.AddressID = a.ID)
SET c.Name='$Name',c.RealtorID='$RealtorID', c.Phone='$Phone', a.Name='$Address_Name', a.Street='$Street', a.City='$City', a.Province='$Province',
  a.Country='$Country', a.Postal_Code='$Postal_Code' WHERE c.ID = '$id'")
...
$query = "SELECT Client.Name as Name, Phone, Address.Name as aName, Street, City, Province, Country, Postal_Code, RealtorID  FROM  Client LEFT JOIN Address on Client.AddressID = Address.ID WHERE Client.ID='$id'";
...
$RealtorID = mysqli_real_escape_string($conn, htmlspecialchars($row['RealtorID']));


// show form

renderForm($id, $Name, $Phone, $Address_Name, $Street, $City, $Province,$Country, $Postal_Code,$RealtorID,$RealtorIDResult, '');
